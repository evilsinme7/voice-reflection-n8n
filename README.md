...
一个"随想随记"的个人感悟工具。在 iPhone 上一键唤起语音听写，说一句话，内容会被自动润色（加标点、改错别字、提炼标题与标签），归档到 Notion 数据库；每周六自动生成一份结构化的"本周思考回顾"。
...
┌─────────────┐     ┌──────────────────────── n8n Workflow A ────────────────────────┐
│  iOS 快捷指令 │     │                                                                  │
│  语音听写     │────▶│  Webhook(鉴权) → 整理字段 → AI润色(DeepSeek) → 解析 → 写入Notion  │
│  (背面轻点)   │ POST│                                                                  │
└─────────────┘     └──────────────────────────────────────────────────────────────┘
                                                                            │
                                                                            ▼
